...
contract AzizaRegistry { 
    ...
    function checkSellingAllowed(address _seller)  external view returns (checkReasonCode reason) 
    {
        ...
        /* check contract state */
        if (contractPaused) {             return checkReasonCode.FAILED_CONTRACT_PAUSED;     }

        if (contractState == ContractStateType.CLOSED) {return checkReasonCode.FAILED_CONTRACT_CLOSED; }

        if (investor[_seller].isConfiscation) { return checkReasonCode.FAILED_FROM_ACCT_CONFISCATED; }   
        if (investor[_seller].isLocked) { return checkReasonCode.FAILED_FROM_ACCT_LOCKED; }
        if (investor[_seller].isFrozen) { return checkReasonCode.FAILED_FROM_ACCT_FROZEN; }
        if (investor[_seller].isVesting) { return checkReasonCode.FAILED_FROM_ACCT_VESTING; }
    ...
    constructor (address _reserveAccount, 
        address _developerAccount ) public        
    {
